    ...
    class Spec < Command
      class Lint < Spec
        ...
        def initialize(argv)
          ...
          @source_urls     = argv.option('sources', 'https://github.com/hCODE-FPGA/Specs.git').split(',')
          ...
          @podspecs_to_lint ||= begin
            ...
            @podspecs_paths.each do |path|
              if path =~ %r{https?://}
                ...
                files += Pathname.glob(pathname + '**/*.spec{.json,}')
                raise Informative, 'No specs found in the current directory.' if files.empty?
              else
                files << (pathname = Pathname.new(path))
                raise Informative, "Unable to find a spec named `#{path}'." unless pathname.exist? && path.include?('.spec')
              end
            end
            files
          end
